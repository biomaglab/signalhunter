...
function varargout = dialog_detect_emganalysis(handles)
% DIALOG_DETECT_EMGANALYSIS Creates GUI panel and controls when axes Button Down Fcn is assessed
...
hstr(2,1) = uicontrol(panel_graph, 'String', num2str(emg_lat,'%.3f'), 'Style', 'text', ...
    'BackgroundColor', 'w', 'FontSize', 10, 'FontWeight', ...
    'bold', 'Units', 'normalized', 'HorizontalAlignment', 'center');
set(hstr(2,1), 'Position', str_2_min_pos);

% static text for maximum latency selection
hstr(2,2) = uicontrol(panel_graph, 'String', num2str(emg_end,'%.3f'), 'Style', 'text', ...
    'BackgroundColor', 'w', 'FontSize', 10, 'FontWeight', ...
    'bold', 'Units', 'normalized');
...
% push button for absence of emg selection
...
handles = callback_detect_emganalysis(handles, 1);
...
handles = callback_detect_emganalysis(handles, 2);

% Update handles structure
guidata(hObject, handles);

function pb_detect_3_Callback(hObject, ~)
% Callback - button for emg absence
handles = guidata(hObject);

handles = callback_detect_emganalysis(handles, 3);
...
handles = callback_detect_emganalysis(handles, 4);
